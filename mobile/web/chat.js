(function () {
    const appConfig = window.XYRON_CONFIG || {};
    ...
    const MISSING_NATIVE_BACKEND_URL = "https://your-deployed-chat-backend.example.com";
    ...
    function getConfiguredBackendOrigin() {
        if (typeof appConfig.backendOrigin !== "string") {
            return null;
        }

        const trimmedValue = appConfig.backendOrigin.trim();
        return trimmedValue.length > 0 ? trimmedValue.replace(/\/+$/, "") : null;
    }

    function isNativePlatform() {
        return Boolean(appConfig.nativePlatform);
    }

    function hasUsableNativeBackendOrigin() {
        const configuredOrigin = getConfiguredBackendOrigin();
        return Boolean(configuredOrigin) && configuredOrigin !== MISSING_NATIVE_BACKEND_URL;
    }
    ...
    function buildClient(tempUserId, joinToken) {
        ...
        if (!wsBaseUrl) {
            appendSystemLine("native app backend URL missing in mobile-config.js");
            throw new Error("Native backend URL is not configured");
        }

        const url = `${wsBaseUrl}/ws/chat?tempUserId=${encodeURIComponent(tempUserId)}&joinToken=${encodeURIComponent(joinToken)}`;
    ...
    async function connect(event) {
        ...
        if (isNativePlatform() && !hasUsableNativeBackendOrigin()) {
            appendSystemLine("set your deployed backend URL in mobile-config.js before building the app");
    ...
    async function bootstrapSession(tempUserId, joinToken) {
        try {
            ...
            const response = await fetch(`${httpBaseUrl}/api/session/bootstrap`, {
                method: "POST",
                headers: {
    ...
    if ("serviceWorker" in navigator && window.location.protocol.startsWith("http")) {
        window.addEventListener("load", () => {
            navigator.serviceWorker.register("/service-worker.js").catch(() => {
                appendSystemLine("offline support unavailable");
            });
        }, { once: true });
    }
